...
class MongoDocumentRepository(BaseDocumentRepository, BaseMongoDBRepository):
    ...
    async def add_new_version(self, document_id: str, version: Version) -> None:
        await self._collection(document_id).insert_one(version.model_dump())

    async def get_last_version(self, document_id: str) -> Version | None:
        document = await self._collection(document_id).find_one(sort=[("_id", -1)])
        if document:
            return convert_version_mongo_to_entity(version=document["version"])
        return
...
if __name__ == '__main__':
    ...
    async def main():
        ...
        version = Version(parent_document_id=document.id, customer_id=document.customer.telegram_id, performer_id=document.performer.telegram_id, version_url="sfsfsd")
