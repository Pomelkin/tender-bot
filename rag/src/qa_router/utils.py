...
from qa_router.prompts import USER_PROMPT_QA, SYSTEM_PROMPT_QA
from qa_router.schemas import Input
from vector_db.queries import search_relevant_chunks
...
async def create_completion_with_context(context: str, query: str) -> str:
    messages = [{"role": "user", "content": SYSTEM_PROMPT_QA + USER_PROMPT_QA.format(query=query, context=context)}]
...
async def create_base_completion(messages: list) -> str:
    ...
    response = await openai_client.chat.completions.create(model="google/gemma-2-9b-it",
                                                           messages=messages,
                                                           temperature=0.0,
                                                           max_tokens=1024,
                                                           timeout=60 * 5)
