...
from vector_db.core import qdrant_instance
...
async def create_collection(file_name: str):
    await qdrant_instance.create_collection(file_name, vectors_config=VectorParams(size=1024,
...
async def search_relevant_chunks(vault_id: str, vector: list[float], top_k: int) -> list[SearchResult]:
    response = await qdrant_instance.search(collection_name=vault_id,
                                            query_vector=vector,
                                            limit=top_k)
    search_result = [SearchResult(page_content=x.payload['page_content']) for x in response]
